...
const wf_env_url = `${origin}/wf_env`
...
const envMethods = {
    ...
    wf_env: async () => {
        try {
            const response = await fetch(wf_env_url)
            if (response.ok) {
                ...
                console.warn(`Wf: the HTTP request for ${wf_env_url} did not succeed: status: ${response.status}`)
                tryNextMethod()
            }
        } catch (err) {
